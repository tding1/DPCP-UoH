function [normals, groups, elapsed_time] = IHL_DPCP(method, Xtilde, init_normals, n, t_budget, threshold, maxiter)
% Iterative Hyperplane Learning via DPCP
...
elseif strcmp(method, 'dpcp-d')
    for i = 1 : n
        mask = GROUPS == i;
        if ~any(mask)
            NORMALS(:,i) = normc(randn(D, 1));
        else
            [~, normal, ~] = solvers.DPCP.DPCP_d(X(:,mask), 2/sqrt(n), t_budget);
            NORMALS(:,i) = normal;
        end
    end
elseif strcmp(method, 'dpcp-irls')
    for i = 1 : n
        mask = GROUPS == i;
        if ~any(mask)
            NORMALS(:,i) = normc(randn(D, 1));
        else
            [normal,~,~] = solvers.DPCP.DPCP_IRLS(X(:,mask), t_budget);
            NORMALS(:,i) = normal;
        end
    end
elseif strcmp(method, 'ransac')
    for i = 1 : n
        ...
        if ~any(mask)
            ...
            weights = ones(1, length(mask(mask==1)));
            normal = solvers.RANSAC.RANSAC_PCA(X(:,mask), D-1, threshold, t_budget, weights);
            NORMALS(:,i) = normal;
        end
    end
elseif strcmp(method, 'kss')
    for i = 1 : n
        mask = GROUPS == i;
        if ~any(mask)
            NORMALS(:,i) = normc(randn(D, 1));
